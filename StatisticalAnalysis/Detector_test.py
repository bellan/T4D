...
for (y,name) in zip(ys,names):
    ...
    ax.legend()
    figure.savefig(f"../figures/StatisticalAnalysis/Detector test {name}.pdf")
    plt.close(figure)


ys = [pull_mesmo_t, pull_mesmo_x, pull_mesmo_y]
names = ["pull_mesmo_t", "pull_mesmo_x", "pull_mesmo_y"]
labels = ["Pull on timing", "Pull on position", "Pull on postition"]
for (y,name, lab) in zip(ys,names, labels):
    figure, ax = plt.subplots()
    ...
    figure.savefig(f"../figures/StatisticalAnalysis/Detector test {name}.pdf", bbox_inches="tight")
    plt.close(figure)
